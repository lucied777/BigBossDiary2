...
import com.example.bigbossdiary2.databinding.FragmentSeleagueBinding
...
class SeleagueFragment : Fragment(), AlertSaveDialog.OnAlertSaveListener {

    private lateinit var  realm : Realm
    private var _binding : FragmentSeleagueBinding? = null
    private val binding: FragmentSeleagueBinding get() = _binding!!
    ...
    override fun onCreateView(
        ...
        _binding = FragmentSeleagueBinding.inflate(inflater,container,false)
        ...
        for (teamNumber in 6..11){
            ...
            if (realmTeamGrade != null){ //チームデータが一個でもあるならインスタンスを作る
                ...
                var totalWin = realm.where<realmTeamGrade>()
                    ...
                    .between("leagueMode", leagueInter, leagueMode).findAll().size
                ...
                var totalGetPoint = realm.where<realmTeamGrade>()
                    .equalTo("teamNumber",teamNumber).between("leagueMode", leagueInter, leagueMode)
    ...
    fun makeListTeamGradeAppend(listData : List<TeamGrade>) {
        ...
        // for (teamNumber in 6..11) { listTeamGradeAppendIn.add(listAppendIn[teamNumber]) }

        for (teamNumber in 6..11) {
            listTeamGrade.add(listData[teamNumber -6] + listDataAppend[teamNumber] )
        }
...
//    }
    ...
    fun onLongClickRecycleListener(teamGrade: TeamGrade){
        ...
        binding.apply {
            ...
            //選択されたチームナンバーを取得
            position = teamGrade.teamNumber -6
