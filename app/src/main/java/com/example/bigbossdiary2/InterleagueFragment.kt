...
import com.example.bigbossdiary2.databinding.FragmentInterleagueBinding
...
class InterleagueFragment : Fragment(),AlertSaveDialog.OnAlertSaveListener {

    private lateinit var  realm : Realm
    private var _binding : FragmentInterleagueBinding? = null
    private val binding: FragmentInterleagueBinding get() = _binding!!
    ...
    private var listTeamGradeFinal = ArrayList<TeamGrade>() //
    ...
    interface OnInterEditSaveListener{
        fun onInterEditSaveClick()
    }
    private lateinit var onInterEditSaveListener : OnInterEditSaveListener

    //////////////////////////////////////////////////////////////////////////////////////////////////////

    override fun onAttach(context: Context) {
        super.onAttach(context)
        if (context is OnInterEditSaveListener)
            onInterEditSaveListener = context
    }
    ...
    override fun onCreateView(
        ...
        _binding = FragmentInterleagueBinding.inflate(inflater,container,false)
    ...
    fun makeListTeamGrade() {
        ...
        //arryayList<TeamGrade>の初期化
        listTeamGrade = ArrayList<TeamGrade>()

        //　listTeamGrade　を作成

        for (teamNumber in 0..11){
            ...
            if (realmTeamGrade != null){ //チームデータが一個でもあるならインスタンスを作る
                ...
                var totalWin = realm.where<realmTeamGrade>()
                    ...
                    .equalTo("leagueMode", leagueMode).findAll().size
                ...
                var totalGetPoint = realm.where<realmTeamGrade>()
                    .equalTo("teamNumber",teamNumber).equalTo("leagueMode",leagueMode)
                ...
                listTeamGrade.add(TeamGrade(teamNumber,totalWin,totalLose,totalDraw,totalGetPoint.toInt(),totalLostPoint.toInt(),totalHomerun.toInt()))

            }else{ //データが無いなら全て０のデータを作成
                listTeamGrade.add(TeamGrade(teamNumber,0,0,0,0,0,0 ))
            }
        }
        makeListTeamGradeAppend()
    }

    ///////////////////////////////////////////////////////////////////////////////////////////////
    //　データベース realmTeamGradeAppend　から listData に追加データを含めたデータを作成

    fun makeListTeamGradeAppend() {
        ...
        // ※listTeamGradeに各モードのエディットモードで作成したデータを含めたデータ

        // leagueMode = 1
        var realmResult = realm.where<realmTeamGradeAppend>()
            .equalTo("leagueMode",leagueMode).equalTo("year",year).findAll().sort("teamNumber")

        var listDataAppend = ArrayList<TeamGrade>()
...
//    }
    ...
    override fun onAlertSaveClick() {
        ...
        binding.apply {
            ...
            onInterEditSaveListener.onInterEditSaveClick()
            // makeListTeamGrade()
        }
